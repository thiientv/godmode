#!/usr/bin/env python3
"""Deterministic P0 orchestration primitives with dependency and resource-aware concurrency."""
...
def ready_nodes(graph: dict) -> list[str]:
    nodes = graph.get("nodes", []); states = {n["id"]: n.get("state", "pending") for n in nodes}
...
def _resources(node: dict) -> set[str]:
    resources = node.get("resources", {})
    if not isinstance(resources, dict): return set()
    files = resources.get("files", [])
    return {f"file:{x}" for x in files if isinstance(x, str) and x} | {f"lock:{x}" for x in resources.get("exclusive", []) if isinstance(x, str) and x}

def _non_conflicting(nodes: dict[str, dict], ready: list[str]) -> list[str]:
    selected: list[str] = []; used: set[str] = set()
    for node_id in ready:
        resources = _resources(nodes[node_id])
        if resources & used: continue
        selected.append(node_id); used |= resources
    return selected

def _run_one(node: dict, runner: Callable[[dict], object]) -> tuple[str, object, str | None]:
    try:
        result = runner(node)
        if result is False: raise RuntimeError("runner returned false")
...
def execute(graph: dict, runner: Callable[[dict], object], policy: RetryPolicy | None = None, max_workers: int = 4) -> dict:
    """Run ready nodes concurrently while preventing declared resource conflicts."""
    ...
    while True:
        ...
        if not ready: break
        batch = _non_conflicting(nodes, ready)
        for node_id in batch:
            attempts[node_id] += 1; nodes[node_id]["state"] = "running"
            events.append({"node": node_id, "event": "started", "attempt": attempts[node_id]})
        if not batch: break
        progressed = False
        with ThreadPoolExecutor(max_workers=min(workers, len(batch)), thread_name_prefix="godmode") as pool:
            futures = {pool.submit(_run_one, nodes[node_id], runner): node_id for node_id in batch}
